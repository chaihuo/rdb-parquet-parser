...
import model.{CFDataAsList, ParquetTable}
 ...
 */
class MySQLCFData2ParquetParser(OutputPath: String, SchemaFilePath: String, MySQLConnString: String, user: String, pwd: String, tableName: String, MysqlPrimaryColumn: String, limit: Int) extends  Runnable{
      ...
      while (parsedCount <= RowCount) {
        ...
        try {
          while ((rs.next)) {
            ...
            writer.write(CFDataAsList.tableList(rs))
            //        lineNumber += 1
          }
          parsedCount += limit
        }
